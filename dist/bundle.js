...
myPluginLoadEvent(function () {
  ...
  var appBody = d3.select('#App-body');
  ...
  var handleDatas = function handleDatas(responseDatas) {
    ...
    var description = descSplit.map(function (sentence, index) {
      if (index > 0) return '<p>' + sentence + '</p>';
    }).join("");
    ...
    var yMax = d3.max(parseDatas, function (d) {
      return d.gdp;
    });
    var yScale = d3.scaleLinear().domain([0, yMax]).range([height - margin.bottom, margin.top]);

    var heightScale = d3.scaleLinear().domain([0, yMax]).range([0, height - margin.bottom - margin.top]);
    ...
    // render rectangles
    svg.selectAll('rect').data(parseDatas, function (d) {
      return d.date;
      ...
      return heightScale(d.gdp);
    }).attr('fill', 'steelblue').on('mouseover', function (d) {
      var rect = d3.event.target;
      d3.select(rect).attr('fill', '#fecc00');
      clearTimeout(_this.unsetHoverTimeout);
      var date = d3.timeFormat("%B %Y")(d.date);
      tooltip.style('left', d3.event.pageX + 5 + "px").style('top', d3.event.pageY - 55 + "px").html('<p class="gdp">' + d.gdp.toFixed(2) + '</p><p class="date">' + date + '</p>').transition().duration(100).style('opacity', 0.9);
